...
if (-not (Test-Command "arm-none-eabi-gcc")) {
...
if ([int]$boardChoice -eq ($Boards.Count + 1)) {
    Write-Host 'Cancelled.'
    exit 0
}

if ([int]$boardChoice -lt 1 -or [int]$boardChoice -gt $Boards.Count) {
...
if ($buildTypeChoice -eq "2") {
    ...
    if ([int]$driverChoice -eq ($FixedDrivers.Count + 1)) {
        Write-Host 'Cancelled.'
        exit 0
    }

    if ([int]$driverChoice -lt 1 -or [int]$driverChoice -gt $FixedDrivers.Count) {
        Write-Host 'Invalid choice.'
        exit 1
    }
...
if ($OGXM_FIXED_DRIVER) {
    $fixedInfo = " fixed=$OGXM_FIXED_DRIVER"
}

$s2Info = ""

if ($OGXM_SWITCH2_HID_RAW_LOG) {
    $s2Info = " switch2_raw_hid=ON"
}
